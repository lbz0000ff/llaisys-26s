...
void launchEmbedding(
    ...
    embeddingKernel<<<gridSize(numel), CUDA_BLOCK_SIZE>>>(
        reinterpret_cast<T *>(out),
        reinterpret_cast<const int64_t *>(index),
        reinterpret_cast<const T *>(weight),
        numel,
        embedding_dim);
    checkCudaLaunch("embedding kernel");
}
} // namespace
...
void embedding(std::byte *out, const std::byte *index, const std::byte *weight,
    ...
    case LLAISYS_DTYPE_BF16:
        return launchEmbedding<__nv_bfloat16>(out, index, weight, index_count, embedding_dim);
    default:
