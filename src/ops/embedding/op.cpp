...
#include "musa/embedding_musa.cuh"
#endif
...
void embedding(tensor_t out, tensor_t index, tensor_t weight) {
    ...
    case LLAISYS_DEVICE_NVIDIA:
        return nvidia::embedding(out->data(), index->data(), weight->data(), out->dtype(), index->numel(),
                                 weight->shape()[1]);
#endif
#ifdef ENABLE_MUSA_API
    case LLAISYS_DEVICE_MUSA:
        return musa::embedding(out->data(), index->data(), weight->data(), out->dtype(), index->numel(),
                                 weight->shape()[1]);
#endif
    default:
