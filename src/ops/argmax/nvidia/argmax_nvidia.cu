...
__global__ void argmaxKernel(int64_t *max_idx, T *max_val, const T *vals, size_t numel) {
    __shared__ float shared_values[CUDA_BLOCK_SIZE];
    __shared__ size_t shared_indices[CUDA_BLOCK_SIZE];
...
void argmax(std::byte *max_idx, std::byte *max_val, const std::byte *vals,
    ...
    case LLAISYS_DTYPE_BF16:
        return launchArgmax<__nv_bfloat16>(max_idx, max_val, vals, numel);
    default:
