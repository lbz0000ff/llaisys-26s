...
void launchRearrange(
    ...
    CHECK_ARGUMENT(shape.size() <= MAX_TENSOR_DIMS, "CUDA rearrange supports at most 8 dimensions");
    TensorIndexMeta meta{};
    ...
    rearrangeKernel<<<gridSize(numel), CUDA_BLOCK_SIZE>>>(
        reinterpret_cast<T *>(out),
        reinterpret_cast<const T *>(in),
        numel,
        meta);
    checkCudaLaunch("rearrange kernel");
}
} // namespace
...
void rearrange(std::byte *out, const std::byte *in, llaisysDataType_t dtype,
    ...
    case LLAISYS_DTYPE_BF16:
        return launchRearrange<__nv_bfloat16>(out, in, shape, strides);
    default:
